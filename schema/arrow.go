...
func ConvertArrowToParquetSchema(schema *arrow.Schema) ([]string, error) {
	...
	for k, v := range schema.Fields() {
		...
		case arrow.FixedWidthTypes.Timestamp_ms.Name():
			timestamp := fieldType.(*arrow.TimestampType) // 'ok' check is not needed, can only be arrow.TimestampType
		...
		case "month_day_ms_interval":
			...
			const intervalMetaDataTemplate = "name=%s, type=FIXED_LEN_BYTE_ARRAY, convertedtype=INTERVAL, logicaltype=INTERVAL, length=12, omitstats=true"
